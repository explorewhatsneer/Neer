...
// ─── 3. CAM EFEKTİ (GLASSMORPHISM) YARDIMCILARI ────────────────

class NeerGlass {
  NeerGlass._();

  /// Hafif cam efektli container (Kartlar, Paneller)
  static BoxDecoration card({required bool isDark}) {
    return BoxDecoration(
      gradient: isDark ? NeerGradients.cardGlassDark : NeerGradients.cardGlassLight,
      borderRadius: BorderRadius.circular(24),
      border: Border.all(
        color: isDark ? NeerColors.glassBorderDark : NeerColors.glassBorder,
        width: 1,
      ),
      boxShadow: [
        BoxShadow(
          color: isDark
              ...
              : NeerColors.primary.withValues(alpha: 0.06),
          blurRadius: 24,
  ...
  /// Güçlü cam efekt (Popup'lar, Sheet'ler)
  static BoxDecoration panel({required bool isDark}) {
    return BoxDecoration(
      gradient: isDark ? NeerGradients.cardGlassDark : NeerGradients.cardGlassLight,
  ...
  /// Yuvarlak cam buton (FAB, İkon butonlar)
  static BoxDecoration circleButton({required bool isDark}) {
    return BoxDecoration(
      color: isDark
  ...
  /// Chip / Pill stili (Filtreler, Etiketler)
  static BoxDecoration chip({required bool isDark, Color? tintColor}) {
    return BoxDecoration(
      color: isDark
          ? (tintColor ?? NeerColors.primary).withValues(alpha: 0.15)
          : (tintColor ?? NeerColors.primary).withValues(alpha: 0.08),
      borderRadius: BorderRadius.circular(100), // Tam yuvarlak
      border: Border.all(
        color: (tintColor ?? NeerColors.primary).withValues(alpha: isDark ? 0.2 : 0.15),
        width: 1,
      ),
    );
  }

  /// Input alanı (Arama, Mesaj yazma)
  static BoxDecoration input({required bool isDark}) {
    return BoxDecoration(
      color: isDark
  ...
  /// Blur filtresi — widget'ları sarmak için
  static ImageFilter get blurLight => ImageFilter.blur(sigmaX: 20, sigmaY: 20);
  static ImageFilter get blurHeavy => ImageFilter.blur(sigmaX: 40, sigmaY: 40);
  static ImageFilter get blurSubtle => ImageFilter.blur(sigmaX: 10, sigmaY: 10);
}
...
/// Cam efektli container — her yerde kullanılabilir
      ...
      child: ClipRRect(
        ...
        child: BackdropFilter(
          ...
          child: GestureDetector(
            onTap: onTap,
            child: Container(
              padding: padding ?? const EdgeInsets.all(NeerSpacing.lg),
              decoration: BoxDecoration(
                gradient: isDark
                    ? NeerGradients.cardGlassDark
                    : NeerGradients.cardGlassLight,
                borderRadius: BorderRadius.circular(borderRadius),
                border: Border.all(
                  color: isDark
                      ? NeerColors.glassBorderDark
                      : NeerColors.glassBorder,
                  width: 1,
                ),
