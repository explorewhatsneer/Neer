# P2 — Profile Header Yeniden Yazımı

P1 tamamlandıktan sonra uygula.

---

## 1. pubspec.yaml — palette_generator paketi ekle

```yaml
dependencies:
  # mevcut paketlerin altına ekle:
...
## 2. ProfileHeaderBackground — YENİ widget

`lib/widgets/profile/profile_header.dart` dosyasının en üstüne import ekle:
```dart
import 'dart:ui';
import 'package:cached_network_image/cached_network_image.dart';
```

Sonra bu widget'ı ekle:
...
## 3. ProfileHeader — yeni içerik düzeni

Mevcut `ProfileHeader` widget'ını tamamen şununla değiştir:
            ...
            Row(
              children: [
                GestureDetector(
                  ...
                  child: _StatChip(value: followersCount, label: 'takipçi'),
                ),
                _StatSep(),
                GestureDetector(
                ...
                GestureDetector(
                  ...
                  child: _StatChip(value: '—', label: 'arkadaş'),
                ),
              ],
            ),
          ],
...
## 4. _ProfileAvatar — Hero animasyonu ile

`profile_header.dart`'a ekle:
...
## 5. profile_screen.dart — SliverAppBar güncelle

```dart
// SliverAppBar parametreleri:
SliverAppBar(
  expandedHeight: 220,  // 300'den küçüldü
  pinned: true,
  floating: false,
  ...
  // flexibleSpace — ProfileHeaderBackground ile sar:
  ...
  // Collapsed header — mevcut kod aynı, sadece arka plan güncellendi
  title: AnimatedOpacity( ... ), // mevcut kod değişmiyor
)
```

Mevcut `GlassButton.appBar` çağrıları `actions:` içinden kaldır — butonlar artık `ProfileHeader` içinde.

---

## KONTROL
- [ ] `flutter analyze` — sıfır hata
- [ ] Header PP blur arka planlı görünüyor
- [ ] Edit/Settings butonları isim hizasında (sağda)
- [ ] NeerScore edit butonlarının altında hizalı
- [ ] Takipçi/takip/arkadaş sol hizalı, tıklanabilir
- [ ] Avatar tıklanınca blur arka planlı büyüyor (Hero animasyonu)
- [ ] Collapsed header arka planı koyu (önceki içerik görünmüyor)
